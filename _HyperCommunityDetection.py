...
from other.h_louvain.h_louvain import hLouvain as hl
...
class HyperCommunityDetect:
    ...
    @staticmethod
    def BeliefPropagation(hsbm, args, return_free_energy=False, parameter_saved=False):
        ...
        arg.dropout = args["dropout"] if "dropout" in args else 0.75
        arg.mp_patience = args["mp_patience"] if "mp_patience" in args else 50
        arg.mp_thresh = args["mp_thresh"] if "mp_thresh" in args else 1e-5
        main0(arg)
        result = np.load(args["save_dir"]+"/inferred_params.npz")
        partition_vec = np.argmax(result["log_marginals"], axis=1)
        if return_free_energy:
            return partition_vec, args["q"], result["best_free_energy"]
        else:
    ...
    def modularity(H):
        start = time.time()
        hyper_g = hnx.Hypergraph.from_incidence_matrix(H.toarray())
        hL = hl(hyper_g, hmod_tau=1, resolution = 1, random_seed = 123)
        alphas = []
        c = 0.3
        b = 0.8
        for i in range(30):
        ...
        print(f"Modularity detect take: {time.time() - start}s")
        partition_vecs = np.array(list(range(H.shape[0])))
        for ci, c in enumerate(A):
            for node in c:
                partition_vecs[node] = ci
        num_groups = len(A)
        return partition_vecs, num_groups
    
    @staticmethod
    def SCLaplacian(hsbm, num_groups):
        L = hsbm.get_operator("L")
        evals_L, evecs_L = eigsh(L, num_groups, which='SA', tol=1e-6)
        combined_evecs = evecs_L
        cluster = KMeans(n_clusters=num_groups, n_init=20)
        cluster.fit(combined_evecs)
        partition_vecs = cluster.predict(combined_evecs)
        return partition_vecs, num_groups
