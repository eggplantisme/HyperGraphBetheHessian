...
class WeightedBetheHessian(BetheHessian):
    ...
    def build_operator(self):
        ...
        A = test_sparse_and_transform(A)
        d = csr_array((A**2).multiply((csr_array(self.r ** 2 * np.ones((n, n))) - A ** 2).power(-1))).sum(axis=1).flatten().astype(float)
        d = diags(d, 0)
        d = d + csr_array(np.identity(n))
        B = d - csr_array((self.r*A).multiply((csr_array(self.r ** 2 * np.ones((n, n))) - A ** 2).power(-1)))
