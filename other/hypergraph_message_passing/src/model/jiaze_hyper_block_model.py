...
from .kappa import compute_C_prime, compute_C_third
from .numerical import hyperedge_pi, sparse_reduce_lse
from .jiaze_dynamic_updates import (
    compute_eta_dynamic_programming,
    compute_eta_tilde_dynamic_programming,
)
...
class HyperBlockModel:
    ...
    communities, specified by an array of assignments t.
    The communities interact through a series affinity tensor p. Together, the community
    ...
    def __init__(
        ...
        p: symmetric tensor dict of community interaction probabilities.
            If specified, this matrix is used as initialization for EM inference,
            otherwise it is initialized at random.
            The tensor has shape (K, ..., K), where K is the number of communities, and
        ...
        hye_sizes: all possible order of hyperedges
        """

        # Model related attributes
        self.cin, self.cout = dict(), dict()  # cin and cout for each order of hyperedges
    ...
    @property
    def c(self):
        """Return the rescaled affinity tensor dict c, defined as
        .. math::
            c[key] = N p[key]
        where N is the number of nodes and p the affinity matrix.
        """
        return dict({key: self.p[key] * self.N ** (key - 1) for key in self.p.keys()})
    ...
    def _parallel_message_passing_step(
        ...
        new_hye_to_node = [
            TYPE_HYE_TO_NODE(x)
            for x in compute_eta_dynamic_programming(
    ...
    def compute_external_field(self) -> np.array:
        ...
        """
        sum_marginal = np.sum(np.exp(self.log_marginals), axis=0)
        external_field = np.zeros(np.shape(sum_marginal))
        for hye_size in self.hye_sizes:
            temp = (self.cin[hye_size] - self.cout[hye_size]) / (self.N ** (hye_size-1) * math.factorial(hye_size-1))
            external_field += self.cout[hye_size] + temp * np.power(sum_marginal, hye_size-1)
        assert external_field.shape == (self.K,)
    ...
    def free_energy(self, hypergraph: IncidenceHypergraph) -> float:
        ...
        inc = hypergraph.get_binary_incidence_matrix()
        hye_dims = inc.sum(axis=0)

        # Node-related addends.
        f_i = [
            (x - inc.multiply(hye_dims-1) * np.log(N)).tocsc().dot(ones) - external_field[k]
            for k, x in enumerate(
                compute_eta_dynamic_programming(hypergraph=hypergraph, model=self)
            )
        ]
        assert len(f_i) == K
        ...
        # First addend.
        first_addend = compute_eta_tilde_dynamic_programming(
            hypergraph=hypergraph, model=self
        )
        first_addend = ((hye_dims - 1) * (np.log(first_addend) - np.log(N)*(hye_dims-1))).sum()

        # Second addend.
        second_addend = 0
        for hye_size in self.hye_sizes:
            second_addend += (1-hye_size) * self.cout[hye_size]
            col_sum_marginal = np.sum(np.exp(self.log_marginals), axis=0)
            temp = np.sum(np.power(col_sum_marginal, hye_size))
            second_addend += (1-hye_size)*(self.cin[hye_size]-self.cout[hye_size])/(np.power(N, hye_size-1)*math.factorial(hye_size))*temp
    ...
    def _check_params(
        self, n: np.ndarray, p: dict, K: int, N: int, max_hye_size: int | None
    ) -> None:
        ...
        if p is not None:
            for key in p.keys():
                if not np.all(p[key] == p[key].T):
                    raise ValueError(f"The probability tensor p order {key} is not symmetric.")
                if np.any(p[key] > 1) or np.any(p[key] < 0):
                    raise ValueError(
                        f"The probability tensor p order {key} contains values outside "
                        "the (0, 1) interval."
                    )
                if p[key].shape != tuple([K] * key):
                    raise ValueError(f"The tensor p order {key} has shape different from (K, ..., K).")
                diagonal = p[key][np.diag_indices(K, key)]
                mask = np.ones(np.shape(p[key]), dtype=bool)
                mask[np.diag_indices(K, key)] = 0
                off_diagonal = p[key][mask]
                if np.size(np.unique(diagonal)) > 1 or np.size(np.unique(off_diagonal)) > 1:
                    raise ValueError(f"The tensor p order {key} is not with shape (cin-cout)*I+cout*J")
                else:
                    # TODO Here only consider consistent cin, cout for each order
                    self.cin[key] = np.unique(diagonal)[0] * N**(key-1)
                    self.cout[key] = np.unique(off_diagonal)[0] * N**(key-1)

        if p is not None and n is not None:
            for key in p.keys():
                if not p[key].shape == tuple([K] * key):
                    raise ValueError(
                        f"The shapes of n and p order {key} do not match. They need to be respectively "
    ...
    def _random_init_n(self) -> None:
        """Random initialization of the community priors n. TODO by Jiaze"""
        self.n = self.rng.dirichlet(alpha=[100] * self.K)

    def _random_init_p(self) -> None:
        """Random initialization of the affinity matrix p. TODO by Jiaze"""
        K = self.K
        N = self.N
    ...
    def _init_message_passing(
        ...
        def random_prob_init():
            ...
            vals = self.rng.dirichlet(
                [dirichlet_alpha] * len(beliefs), size=len(beliefs[0].data)  # the output size of dirichlet is (size * alpha length)
            )
        ...
        # Infer hye to node as ratio of marginals and node to hye
        log_hye_to_node = []
        for assignment in range(self.K):
