...
class HyperSBM:
    ...
    def get_operator(self, operator='B', r=0):
        ...
        if operator == 'Bi':
            ...
            L = eye(self.n) - D.power(-1/2) @ A @ D.power(-1/2)
            return L
        else:
            pass
    ...
    def get_projection_operator(projection_matrix, operator='WNB', r=0):
        ...
        if operator == 'WNB':
            ...
            # BBT = r * BBT.tanh()
            d = csr_array((BBT ** 2).multiply((csr_array(r ** 2 * np.ones((n1, n1))) - BBT ** 2).power(-1))).sum(axis=1).flatten().astype(
                float)
            d = diags(d, 0)
            d = d + csr_array(np.identity(n1))
            BH = d - csr_array((r * BBT).multiply((csr_array(r ** 2 * np.ones((n1, n1))) - BBT ** 2).power(-1)))
            return BH
